...
$reponse = $conn->query("UPDATE `client` SET `asr`='".$asr."',`tel`='".$tel."',`pass`='".$pass."'
 WHERE  `client`.`ndoss` = '".$ndoss."';"); 
}
?> 
<?php
require "db_conn.php";
$ndoss=$_GET['ndoss'];
 $stmt=$conn->query("select * from client where ndoss='".$ndoss."'");
...
<br>
                    <form class="forms-sample"  method="POST">
                      <div class="form-group">
                        <label>Assuré</label>
                        <input type="text" class="form-control" name="asr"  placeholder="Nom Et Prénom" value="<?php echo $asr?>" >
                      </div>
                    
                      <div class="form-group">
                        <label>Numéro De Téléphone</label>
                        <input type="text" class="form-control"  name="tel"  placeholder="Téléphone" value="<?php echo $tel?>">
                      </div>
                      <div class="form-group">
                        <label>Mot de passe de l'application</label>
                        <input type="text" class="form-control"  name="pass"  placeholder="Mot de passe" value="<?php echo $pass?>">
                      </div>
                     
                      <button type="submit" class="btn btn-gradient-primary me-2" name="submit" >Modifier</button>
                      <a href="http://localhost/gestion_assurance/clientlst.php"class="btn btn-light">
                      Retour</a>
                    </form>
                  </div>
                </div>
